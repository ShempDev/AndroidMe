...
public class AndroidMeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) { //Only create new fragments if none exist.
            //First get the bundle from the main activity.
            Bundle bundle = getIntent().getExtras();
            //Create a BodyPartFragments and set images for each body segment.
            BodyPartFragment headFragment = new BodyPartFragment();
            headFragment.setmImageIds(AndroidImageAssets.getHeads());
            headFragment.setmListIndex(bundle.getInt("headIndex", 0));
            BodyPartFragment bodyFragment = new BodyPartFragment();
            bodyFragment.setmImageIds(AndroidImageAssets.getBodies());
            bodyFragment.setmListIndex(bundle.getInt("bodyIndex", 0));
            BodyPartFragment legFragment = new BodyPartFragment();
            legFragment.setmImageIds(AndroidImageAssets.getLegs());
            legFragment.setmListIndex(bundle.getInt("legIndex", 0));
